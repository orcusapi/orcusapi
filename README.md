...
- Argument-less, read-only functions → `GET /invoke/{name}`, no body.
- Everything else (any function that takes arguments, and/or writes state)
  → `POST /invoke/{name}`, args in the JSON body.
...
`/invoke/{function_name}` below. These work the same on both `GET` and
